resource "aws_instance" "fedora36" {
  count                       = var.make_instances  ? 1 : 0
  ami                         = data.aws_ami.fedora36.id
...
#!/bin/bash
output_log_name="fedora36_provision_output.log"
...
resource "aws_route53_record" "fedora36-dns-record" {
  count   = var.make_instances  ? 1 : 0
  zone_id = aws_route53_zone.private-zone.zone_id
  name    = "fedora36.fogtesting.cloud"
  type    = "CNAME"
  ttl     = "300"
  records = [aws_instance.fedora36[0].private_dns]
}
