resource "aws_instance" "fedora37" {
  count                       = var.make_instances ? 1 : 0
  ami                         = data.aws_ami.fedora37.id
...
#!/bin/bash
output_log_name="fedora37_provision_output.log"
...
resource "aws_route53_record" "fedora37-dns-record" {
  count   = var.make_instances ? 1 : 0
  zone_id = aws_route53_zone.private-zone.zone_id
  name    = "fedora37.fogtesting.cloud"
  type    = "CNAME"
  ttl     = "300"
  records = [aws_instance.fedora37[0].private_dns]
}
