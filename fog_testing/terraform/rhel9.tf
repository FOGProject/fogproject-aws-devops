...
#!/bin/bash
output_log_name="rhel9_provision_output.log"
...
resource "aws_route53_record" "rhel9-dns-record" {
  count   = var.make_instances  ? 1 : 0
  zone_id = aws_route53_zone.private-zone.zone_id
  name    = "rhel9.fogtesting.cloud"
  type    = "CNAME"
  ttl     = "300"
  records = [aws_instance.rhel9[0].private_dns]
}
